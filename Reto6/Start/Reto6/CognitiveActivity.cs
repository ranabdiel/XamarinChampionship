    ...
    public class CognitiveActivity : Activity
    {
		...
		private async void BtnAnalizaFoto_Click(object sender, EventArgs e)
		{
			if (streamCopy != null)
			{
				...
				if (emotions != null) { txtResultado.Text = "---Análisis de Emociones---"; sb.AppendLine(); foreach (var item in emotions) { string toAdd = item.Key + " : " + item.Value + " "; sb.Append(toAdd); } txtResultado.Text += sb.ToString(); btnRegistraResultados.Visibility = ViewStates.Visible; } else txtResultado.Text = "---No se detectó una cara---"; ResultadoEmociones += sb.ToString();
			}
